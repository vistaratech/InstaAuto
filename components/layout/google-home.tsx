...
import { Search, Zap, Instagram, MessageCircle, Shield, Clock, ArrowRight, X, ExternalLink, Sparkles, Check } from "lucide-react"
...
export function GoogleSearchHome() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between select-none">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-8 md:py-16 max-w-2xl mx-auto w-full text-center">
        ...
        <div className="flex items-center gap-2.5 md:gap-3 mt-6 md:mt-8 flex-wrap justify-center">
          <a
            href={instagramAuthUrl}
        ...
        {/* Feature Pills Row */}
        <div className="flex flex-wrap items-center justify-center gap-2 mt-8 md:mt-10 max-w-lg">
          {featurePills.map((pill, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-secondary/50 border border-border/60 text-[11px] font-medium text-muted-foreground select-none"
            >
              {pill.icon}
              {pill.label}
            </span>
